/* Functionality for implementing a random vector without using the algorithm header library.
*/
...
double RandomVector::max(){
    ...
    //  Compute total sum of random array
    for (int i=0; i < this->vect.size(); ++i) {
        if (this->vect[i] > max_value) {
            max_value = this->vect[i];
        }
    }
    return max_value;
}
...
double RandomVector::min(){
    ...
    //  Compute total sum of random array
    for (int i=0; i < this->vect.size(); ++i) {
        if (this->vect[i] < min_value) {
            min_value = this->vect[i];
        }
    }
    return min_value;
}
